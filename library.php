...
if(empty($_GET['vlist'])){
  header("location: 404");
	 ...
	 $cat=explode("_",$cotp);
	  $auth=$cat[0];
	  $catid=$cat[1];
    $chkcid=$DBcon->query("SELECT * FROM  m_cat  WHERE cat_id='$catid' ");
    $cntmid=$chkcid->num_rows;
	if($cntmid!=1){
	header("location: 404");
	}else{
	$videos=$chkcid->fetch_array();
	$cat_id=$videos['cat_id'];
    $cat_name=$videos['cat_name'];
                  ...
                  <div class="row" id="display">
                     <div class="col-md-12">
                        <div class="main-title">
                           ...
                           <h6><?php echo $cat_name; ?> Videos</h6>
                        </div>
                     </div>
						<?php $getvid=$DBcon->query("SELECT m.*,c.cat_name FROM media m, m_cat c WHERE m.cat_id=c.cat_id AND m.cat_id='$cat_id' AND m.m_flag=1 AND m.l_time <='$ctime'"); 
                        ...
                        <div class="video-card">
                           ...
                           <div class="video-card-body">
                              ...
                              <div class="video-page text-success">
                                 <?php  $getcat=$DBcon->query("select cat_name FROM m_cat WHERE cat_id=".$vid['cat_id']."");
